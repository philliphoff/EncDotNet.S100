...
/// <summary>
/// Tests for <see cref="S104DatasetReader.ReadMetadata"/> — the phased
...
/// <c>values</c> arrays.
/// </summary>
public class S104DatasetReaderMetadataTests
{
    [Fact]
    public void ReadMetadata_ExtentAndTimeMatchFullReadCoverageSource()
    {
        ...
        try
        {
            ...
            S104FixtureBuilder.WriteFile(path, values, numLat: 3, numLon: 4, useF64GridAttrs: true, useUnsignedCounts: false);

            DatasetMetadata meta;
            using (var hdf = PureHdfFile.Open(path))
                meta = S104DatasetReader.ReadMetadata(hdf);
    ...
    public void ReadMetadata_SurfacesSpecAndCrs()
    {
        ...
        {
            var values = new[] { new S104FixtureBuilder.SpecRow { WaterLevelHeight = 1.5f, WaterLevelTrend = 2 } };
            S104FixtureBuilder.WriteFile(path, values, numLat: 1, numLon: 1, useF64GridAttrs: false, useUnsignedCounts: false);

            using var hdf = PureHdfFile.Open(path);
            var meta = S104DatasetReader.ReadMetadata(hdf);

            Assert.Equal("S-104", meta.Spec.Name);
    ...
    public void ReadMetadata_DoesNotReadValueArrays_ButFullReadDoes()
    {
        ...
        {
            var values = new[] { new S104FixtureBuilder.SpecRow { WaterLevelHeight = 1.5f, WaterLevelTrend = 2 } };
            S104FixtureBuilder.WriteFile(path, values, numLat: 2, numLon: 2, useF64GridAttrs: true, useUnsignedCounts: false);

            using (var spy = new DatasetReadSpyFile(PureHdfFile.Open(path)))
            {
                _ = S104DatasetReader.ReadMetadata(spy);
                Assert.Equal(0, spy.DatasetReadCount);
            }

            using (var spy = new DatasetReadSpyFile(PureHdfFile.Open(path)))
            {
                _ = S104DatasetReader.Read(spy);
