...
- **Bridges are decomposed into spans** (S-65 Annex B §4.8.10). A `BRIDGE`
  ...
  The collection's lights and warning signal stations are linked to that
  `Bridge` as `theEquipment` of a `StructureEquipment` association: IENC places
  bridge lights on the navigable span and the piers bounding it with no master
  object, and aggregates a bridge's vertical clearance indicators (`sistaw`,
  `catsiw` 16) with it, so collection membership is the only link (IENC
  Encoding Guide 2.4.1, clauses N.1.1 and I.3.3). Other members, such as the
