...
- **Bridges are decomposed into spans** (S-65 Annex B §4.8.10). A `BRIDGE`
  ...
  The collection's lights are linked to that `Bridge` as `theEquipment` of a
  `StructureEquipment` association: IENC places bridge lights on the navigable
  span and the piers bounding it with no master object, so collection
  membership is the only link (IENC Encoding Guide 2.4.1). Other members, such
  as the fenders, notice marks and signal stations IENC collects with a
  bridge, convert on their own. A collection that also holds a navigation line
  ...
  *Bridge spans*, *Aggregated bridges*, *Bridge lights linked* and *Bridge
