...
- **Bridges are decomposed into spans** (S-65 Annex B §4.8.10). A `BRIDGE`
  ...
  or track is a track grouping, not a bridge collection. The summary's *Bridge
  spans*, *Aggregated bridges* and *Bridge lights linked* rows (and the
  report's `bridgeSpansEmitted` / `bridgeAggregationsEmitted` /
  `bridgeEquipmentLinked`) count them.
