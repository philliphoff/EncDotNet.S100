...
- **Bridges are decomposed into spans** (S-65 Annex B §4.8.10). A `BRIDGE`
  ...
  which has no such value), since spans cannot carry a name. When the members'
  geometry doesn't join into one curve or one surface, the members convert one
  by one, as a `Bridge` each, and each light is linked to the nearest of them,
  so no bridge loses its drawn name to a geometry-less `Bridge`. The summary's
