...
| `describe_feature` | Returns spec, feature type, attributes, and (for S-101) resolved geometry for a feature id in a given dataset. Supported specs: S-101 (RCID; result carries a `geometry` block with primitive, bounding box, and coordinates — for MultiPoint `Sounding` features it also carries a `depths` array aligned one-to-one with `coordinates` plus an explicit `depthUnit` of `"metres"` (positive down) so the unit is never inferred), S-102 (`BathymetryCoverage[.01]`), S-104 / S-111 (`WaterLevel`/`SurfaceCurrent[.NN][.Group_KKK]` or bare station identifier), S-124 (`gml:id`), and S-129 (`gml:id` of plan / plan-area / control-point / non-navigable-area). |
