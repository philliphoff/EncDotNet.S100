...
| `query_features` | Returns features from loaded GML vector datasets whose geometry intersects a spatial query. An optional `attributes` filter (a JSON object of `code` → `value`, combined with AND; a `null` value tests presence) narrows results by attribute content — matched case-insensitively against both simple and complex attributes. |
| `find_nearest` | Returns the loaded GML vector features nearest to a lat/lon point, ranked closest-first by great-circle distance to each feature's bounding box (`distanceMeters`, `0` when the point is inside). Optional `spec`, `featureType`, `attributes`, `maxResults` (clamped `1..200`), and `maxDistanceMeters` filters. Answers "what is the closest restricted area to my position?" without paging `query_features` through expanding boxes. |
...
| `get_viewer_state` *(viewer only, read-only)* | Returns a snapshot of the live viewer's render and navigation state: the current `viewport` (WGS-84 bbox plus centre and web-mercator zoom), the active `palette` and `displayCategory`, the global-time `time` clock (current / min / max instants, current index, sample count), the loaded-`datasetCount`, and the simulated `ownShip` kinematics. The read-side counterpart to the `set_*` tools — use it to assert preconditions and verify results without a side-effecting write. Each section is `null` when its subsystem is not yet ready. |
| `pick_feature_at` *(viewer only, read-only)* | Resolves the world point under a pixel `(x, y)` of a `render_to_image`-style capture (`width` / `height` default to render_to_image's `1024×768`) and returns the GML `features` and `datasets` whose bounds contain it — turning a screenshot coordinate into concrete feature ids for precise visual assertions without pixel-diff baselines. Matching is at bounding-box precision; coverage products and S-101 contribute datasets but not features. |
...
* **Read-only** — never mutate viewer state. Safe to call from any
  ...
  `find_nearest`, `query_features`, `sample_coverage`,
  `get_viewer_state` and `pick_feature_at` (which read the live
  navigator / catalog without changing them), `render_to_image` (which
