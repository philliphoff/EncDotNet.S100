...
/// <summary>
/// Adapter that exposes the geometry of an <see cref="S122Dataset"/> through the
/// product-agnostic <see cref="IFeatureGeometryProvider"/> contract used by the
/// unified Mapsui display-list renderer.
/// </summary>
/// <remarks>
/// S-122 feature references are GML identifiers (e.g. <c>NW.GB.123</c>) carried
...
/// curve, then points.
/// </remarks>
public sealed class S122FeatureGeometryProvider : IFeatureGeometryProvider
{
    private readonly Dictionary<string, FeatureGeometry> _byId;

    /// <summary>Builds a provider over the supplied dataset.</summary>
    public S122FeatureGeometryProvider(S122Dataset dataset)
    {
    ...
    private static FeatureGeometry? BuildGeometry(S122Feature feature)
    {
        ...
        if (!feature.Curves.IsDefaultOrEmpty && feature.Curves.Length > 0)
        {
            ...
            // existing S-122 renderer behaviour).
