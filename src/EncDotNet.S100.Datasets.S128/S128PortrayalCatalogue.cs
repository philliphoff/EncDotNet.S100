...
/// <summary>
/// S-128 portrayal catalogue implementing <see cref="IVectorPortrayalCatalogue"/>.
/// Loads XSLT rules, colour palette, symbols, and line styles from a
/// <see cref="PortrayalCatalogueProvider"/>.
/// </summary>
/// <remarks>
/// S-128 (edition 2.0.0) uses purely XSLT-based portrayal rules (no Lua).
/// The bundled <c>main.xsl</c> includes <c>simpleLineStyle.xsl</c>,
/// <c>textStyle.xsl</c>, per-feature templates
/// (<c>ElectronicProduct.xsl</c>, <c>PhysicalProduct.xsl</c>,
/// <c>S100Service.xsl</c>, <c>DistributorInformation.xsl</c>) and a
/// <c>Default.xsl</c> fallback for unhandled feature types.
/// See S-128 § 13 (Portrayal Catalogue).
/// </remarks>
public sealed class S128PortrayalCatalogue : IVectorPortrayalCatalogue
{
    ...
    /// <summary>
    /// Initializes a new <see cref="S128PortrayalCatalogue"/> backed by the given provider.
    /// </summary>
    public S128PortrayalCatalogue(PortrayalCatalogueProvider provider)
    {
    ...
    /// <remarks>
    /// The bundled S-128 (edition 2.0.0) Portrayal Catalogue ships
    /// <c>Day</c>, <c>Dusk</c>, and <c>Night</c> <c>&lt;palette&gt;</c>
    /// blocks in <c>colorProfile.xml</c>; all three are loaded eagerly on
    /// the first <see cref="SwitchPalette"/> call.
    /// </remarks>
    ...
    /// <summary>Gets the ordered list of portrayal rules defined by the S-128 Portrayal Catalogue.</summary>
    ...
    {
        // S-128 uses a single top-level XSLT rule (main.xsl) that includes all sub-templates.
    ...
    /// <summary>
    /// The current S-128 portrayal catalogue ships only XSLT rules, so this
