...
/// <summary>
/// S-127 portrayal catalogue implementing <see cref="IVectorPortrayalCatalogue"/>.
/// Loads XSLT rules, colour palette, symbols, and line styles from a
/// <see cref="PortrayalCatalogueProvider"/>.
/// </summary>
/// <remarks>
/// S-127 Edition 2.0.0 portrayal is XSLT-only (no Lua). The top-level
/// <c>main.xsl</c> rule includes per-feature sub-templates plus a
/// <c>Default.xsl</c> fallback via <c>xsl:include</c>.
/// </remarks>
public sealed class S127PortrayalCatalogue : IVectorPortrayalCatalogue
{
    ...
    /// <summary>Initializes a new <see cref="S127PortrayalCatalogue"/> backed by the given provider.</summary>
    public S127PortrayalCatalogue(PortrayalCatalogueProvider provider)
    {
    ...
    private void EnsurePalettesLoaded()
    {
        ...
        foreach (var item in _provider.Catalogue.ColorProfiles)
        {
            ...
            {
                // S-127 ships a single colorProfile.xml that contains all three palettes.
    ...
    /// <summary>Gets the ordered list of portrayal rules defined by the S-127 Portrayal Catalogue.</summary>
    ...
    {
        // Like S-124, S-127 uses a single top-level XSLT rule (main.xsl) that
        // includes all sub-templates. Only the TopLevelTemplate rule is run;
        // sub-templates are resolved via xsl:include.
    ...
    /// <summary>
    /// S-127 Edition 2.0.0 ships only XSLT rules, so this method always
    /// throws <see cref="KeyNotFoundException"/>. Future editions or vendor
    /// catalogues may include Lua rules; in that case this method will
    /// resolve them via the loaded rule files.
    /// </summary>
    ...
    /// <summary>
    /// An <see cref="XmlResolver"/> that resolves xsl:include / xsl:import
    /// URIs against the portrayal catalogue's embedded assets so the bundled
    /// per-feature XSLT sub-templates and <c>Default.xsl</c> fallback are
    /// reachable from <c>main.xsl</c>.
    /// </summary>
