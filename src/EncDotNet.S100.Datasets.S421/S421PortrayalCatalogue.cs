...
/// <summary>
/// S-421 portrayal catalogue implementing <see cref="IVectorPortrayalCatalogue"/>.
/// Loads XSLT rules, colour palette, symbols, line styles, and area fills
/// from a <see cref="PortrayalCatalogueProvider"/>.
/// </summary>
/// <remarks>
/// S-421 uses purely XSLT-based portrayal rules (no Lua). Top-level templates
/// (e.g. <c>main_Simplified.xsl</c>) include sub-templates via <c>xsl:include</c>.
/// </remarks>
public sealed class S421PortrayalCatalogue : IVectorPortrayalCatalogue
{
    ...
    /// <summary>
    /// Initializes a new <see cref="S421PortrayalCatalogue"/> backed by the given provider.
    /// </summary>
    public S421PortrayalCatalogue(PortrayalCatalogueProvider provider)
    {
    ...
    /// <summary>Gets the ordered list of portrayal rules defined by the S-421 Portrayal Catalogue.</summary>
    ...
    // ── Lua (not used by S-421) ────────────────────────────────────────

    /// <summary>Not supported — S-421 uses XSLT portrayal rules only.</summary>
    /// <exception cref="NotSupportedException">Always thrown.</exception>
    public Script GetLuaScript(string scriptName)
    {
        throw new NotSupportedException("S-421 does not use Lua portrayal rules.");
    }
