...
/// new source goes into (a new one, named, or an existing one) and, for the
/// NOAA ENC feed, which states, Coast Guard districts or regions to include.
/// Nothing is loaded or downloaded except the NOAA catalogue itself.
/// </summary>
internal sealed class AddToLibraryDialogViewModel : ViewModelBase
{
    ...
    public string SourceDescription => _kind == AddToLibraryKind.NoaaFeed
        ? NoaaEncFeedSource.DefaultCatalogUri.AbsoluteUri
        : _path ?? string.Empty;
    ...
    public void Initialize(AddToLibraryKind kind, string? path, Guid? targetCollectionId)
    {
        ...
        _newCollectionName = kind == AddToLibraryKind.NoaaFeed
            ? Strings.Library_NoaaFeed
            : DefaultName(path);
    ...
    /// <summary>Fetches the NOAA catalogue and populates the facet lists.</summary>
    ...
    private bool CanConfirm =>
        ...
        && (_kind != AddToLibraryKind.NoaaFeed || (_catalog is not null && !_isLoading))
        && (_kind == AddToLibraryKind.NoaaFeed || !string.IsNullOrEmpty(_path));
    ...
    private void Populate(
        ...
        IEnumerable<NoaaEncFacetValue> values,
        Func<NoaaEncFacetValue, string> label)
    {
    ...
    {
        foreach (var option in States.Concat(CoastGuardDistricts).Concat(Regions))
...
/// <summary>One selectable NOAA facet value (a state, district or region).</summary>
internal sealed class FacetOptionViewModel : ViewModelBase
{
    private bool _isSelected;

    public FacetOptionViewModel(NoaaEncFacetValue value, string label)
    {
        Value = value.Value;
        Label = label;
    ...
    /// <summary>The facet value (state code, or district/region number).</summary>
