...
/// <summary>
/// Adapter that exposes the geometry of an <see cref="S125Dataset"/> through
...
/// order: surface (with interior rings), the first curve, then points.
/// </remarks>
public sealed class S125FeatureGeometryProvider : IFeatureGeometryProvider
{
    private readonly Dictionary<string, FeatureGeometry> _byId;

    /// <summary>Builds a provider over the supplied dataset.</summary>
    public S125FeatureGeometryProvider(S125Dataset dataset)
    {
    ...
    private static FeatureGeometry? BuildGeometry(S125Feature feature)
    {
