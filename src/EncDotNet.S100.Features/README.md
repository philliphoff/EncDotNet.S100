...
- **`AttributeBinding`**, **`FeatureBinding`**, **`InformationBinding`** — the attributes, feature associations and information associations a type binds. A feature or information binding may admit several target types; `FeatureTypeRefs` / `InformationTypeRefs` list them all, and `FeatureTypeRef` / `InformationTypeRef` give the first.
