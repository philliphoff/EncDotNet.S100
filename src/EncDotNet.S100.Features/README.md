...
- **`FeatureCatalogueManager`** — resolves, parses, and caches a `FeatureCatalogue` per product spec from an injected stream resolver (with an optional bundled `IAssetSource` fallback registered via `SetSource`). `GetContentHash(spec)` returns a memoized lowercase-hex SHA-256 of the *resolved* catalogue XML bytes — reflecting any CLI / settings override the resolver applies, so it is a safe cache-invalidation input where a declared version string could miss an override that changes the file without bumping its version. `SetSource` invalidates the parse, decoder, and content-hash caches for the affected spec.
