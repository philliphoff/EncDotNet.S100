...
- **Bridges are not decomposed into spans.** S-65 Annex B § 4.8.10 converts a `BRIDGE` over navigable water into a `Bridge` plus `SpanFixed` / `SpanOpening` components joined by a `BridgeAggregation` association, with the clearances (`VERCLR`, `VERCCL`, `VERCOP`, `HORCLR`) carried on the spans; a point `BRIDGE` becomes a `Landmark`. The translator emits a single `Bridge` of the source primitive with the `CATBRG`-derived category attributes, so the clearances are not carried on span features as the guidance intends.
