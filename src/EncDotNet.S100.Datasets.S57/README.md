...
- **Simple attributes plus selected complex attributes.** Most attribute rules cover S-57 attributes that map to a *directly feature-bound simple* S-101 attribute. The `information` (`INFORM`/`NINFOM`/`TXTDSC`/`NTXTDS`), `featureName` (`OBJNAM`/`NOBJNM`), `rhythmOfLight` (`LITCHR`/`SIGGRP`/`SIGPER`, first level only), the three date-range complex attributes — `fixedDateRange` (`DATSTA`/`DATEND`), `periodicDateRange` (`PERSTA`/`PEREND`), and `surveyDateRange` (`SURSTA`/`SUREND`) — `zoneOfConfidence` (`CATZOC`), and `surfaceCharacteristics` (`NATSUR`/`NATQUA`) are also assembled, gated on the S-101 Feature Catalogue's per-feature attribute bindings. Assembled complex sub-attributes are validated against the destination simple attribute's *global* enumeration rather than the (sometimes narrower) per-complex `permittedValues` subset, matching the fidelity of the rest of the pipeline. Attributes that are sub-attributes of the *remaining* S-101 **complex** attributes — the nested `signalSequence` (`SIGSEQ`), the `lightSector`/`sectorLimit` sector geometry (`SECTR1`/`SECTR2`/`LITVIS`), and the nested uncertainty complexes of `zoneOfConfidence` — are still left unmapped pending further complex-attribute assembly. `SORDAT`→`reportedDate` is deferred (it binds to only a subset of feature types and the translator has no per-feature binding filter); `SORIND` has no general S-101 equivalent and is intentionally not mapped.
