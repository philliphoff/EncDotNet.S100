...
/// <summary>
/// S-129 portrayal catalogue implementing <see cref="IVectorPortrayalCatalogue"/>.
/// Loads XSLT rules, colour palette, symbols, line styles, and area fills
/// from a <see cref="PortrayalCatalogueProvider"/>.
/// </summary>
/// <remarks>
/// S-129 uses purely XSLT-based portrayal rules (no Lua).
/// The main.xsl rule includes all sub-templates via xsl:include.
/// </remarks>
public sealed class S129PortrayalCatalogue : IVectorPortrayalCatalogue
{
    ...
    private sealed class AssetSourceXmlResolver : XmlResolver
    {
        ...
        public override object? GetEntity(Uri absoluteUri, string? role, Type? ofObjectToReturn)
        {
            ...
            // First, try to resolve by filename against registered rule files
            var ruleFile = _provider.Catalogue.RuleFiles
            ...
            // Fall back to resolving by relative path from the Rules directory.
            // The absolute URI may be rooted at the CWD, so extract the meaningful
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                if (segments[i].Equals("templates", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    try
                    {
                        return _provider.FetchRuleAsync(relativePath).GetAwaiter().GetResult();
                    }
                    catch
                    {
                        // Fall through to filename-only attempt
                    }
                    break;
                }
            }
